...
void benchmark(VglImage *image, size_t rounds, std::function<void(VglImage *, const char *)> save_image) {
  ...
  builder.attach({
    .name = "threshold",
    .func = [&] { vglClNdThreshold(image, output, 30); },
    .post = save_sample
  });
  builder.attach({
    .name = "erode-cube",
  ...
  builder.attach({
    ...
    .func = [&] {
      ...
      for (int i = 2; i <= dimensions; ++i)
        if (i & 0b1) vglClNdErode(output, tmp, strel_cube_array[i]);
        else vglClNdErode(tmp, output, strel_cube_array[i]);
      if (dimensions & 0b1) vglClNdCopy(tmp, output);
    },
    .post = save_sample
  });
  builder.attach({
  ...
  builder.attach({
    ...
    .func = [&] {
      ...
      for (int i = 2; i <= dimensions; ++i)
        if (i & 0b1) vglClNdConvolution(output, tmp, strel_mean_array[i]);
        else vglClNdConvolution(tmp, output, strel_mean_array[i]);
      if (dimensions & 0b1)
