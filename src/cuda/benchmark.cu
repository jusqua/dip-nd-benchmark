...
__global__ void invertKernel(const Image *input, Image *output) {
  ...
  output->data[i] = std::numeric_limits<uint8_t>::max() - input->data[i];
}

__global__ void thresholdKernel(const Image *input, Image *output, uint8_t threshold, uint8_t max_value) {
  size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= input->size) return;
...
inline __global__ void windowMap(Image *input, Image *output, Window *window, const size_t index, Func&& func) {
  ...
  for(size_t window_index = 0; window_index < window->size; ++window_index) {
    if (window->data[window_index] == 0) continue;
    ...
    for(int d = window->dimensions; d >= 1; --d) {
      ...
      if (window_coord[d] < 0) window_coord[d] = 0;
      else if (window_coord[d] > maxv) window_coord[d] = maxv;
...
__global__ void erodeKernel(Image *input, Image *output, Window *window) {
  ...
  windowMap(input, output, window, i, [&](size_t image_index, size_t _) {
...
__global__ void convolveKernel(Image *input, Image *output, Window *window) {
  ...
  float result = 0.0f;
  windowMap(input, output, window, i, [&](size_t image_index, size_t window_index) {
...
void benchmark(VglImage *image, size_t rounds, std::function<void(VglImage *, const char *)> save_image) {
  ...
  const int blocks = (int)((image_size + threadsPerBlock - 1) / threadsPerBlock);
  ...
  builder.attach({
    .name = "threshold",
    .func = [&] {
      thresholdKernel<<<blocks, threadsPerBlock>>>(d_input_img, d_output_img, static_cast<uint8_t>(30), std::numeric_limits<uint8_t>::max());
      cudaDeviceSynchronize();
    },
    .post = save_sample
  });
  ...
  builder.attach({
    ...
    .func = [&] {
      ...
      cudaDeviceSynchronize();
      for (int i = 2; i <= dimensions; ++i) {
        if (i & 0b1) convolveKernel<<<blocks, threadsPerBlock>>>(d_output_img, d_aux_img, d_window_mean_array[i]);
        else convolveKernel<<<blocks, threadsPerBlock>>>(d_aux_img, d_output_img, d_window_mean_array[i]);
        cudaDeviceSynchronize();
      }
      if (dimensions & 0b1) {
        cudaMemcpy(d_output_data, d_aux_data, image_size, cudaMemcpyDeviceToDevice);
